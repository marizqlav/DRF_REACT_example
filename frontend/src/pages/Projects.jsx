...
export default function Projects() {
  //1) crear un estado inicial para guardar las tareas
  const [projects, setProjects] = useState([]);

  //2) llamada a la API para obtener las tareas
  async function getAllProjects() {
    const response = await fetch("https://mario.pythonanywhere.com/api/projects/", {
      method: "GET",
    });
    const data = await response.json();
    //3) setear/convertir la respuesta a formato JSON
    setProjects(data);
  }

  //4) llamamos useEffect para que se ejecute la funcion getAllProjects() cuando se renderice el componente
  useEffect(() => {
    getAllProjects();
  }, []);
